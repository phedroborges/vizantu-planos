...
import { formatApprovalDeadline, isPlanExpired } from "@/lib/approval-deadline";
...
function reviewerHostScript(slug: string, deadlineAt?: string) {
  return `(function(){
    "use strict";
    ...
    function send(){
      ...
      var current=state();
      frame.contentWindow.postMessage({type:"vizantu:identity:state",slug:slug,active:current.active,profiles:current.profiles,deadlineAt:deadlineAt,deadlineLabel:deadlineLabel},"*");
    }
...
export async function GET(_: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  if (result.plan.approvalDeadline && isPlanExpired(result.plan)) return closedPlan(title, result.plan.approvalDeadline);
  ...
  const deadlineBar = deadlineAt
    ? `<aside class="vz-deadline" aria-label="Prazo para aprovação"><div><strong>Prazo para aprovação</strong><span>Até ${escapeHtml(formatApprovalDeadline(deadlineAt))} · depois, aprovação automática</span></div><time id="vz-deadline-countdown" datetime="${escapeHtml(deadlineAt)}">Calculando…</time></aside>`
    : "";
  const frameHeight = deadlineAt ? "calc(100% - 62px)" : "100%";
  const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><style>html,body{width:100%;height:100%;margin:0;overflow:hidden;background:#fff}.vz-deadline{box-sizing:border-box;height:62px;display:flex;align-items:center;justify-content:space-between;gap:24px;padding:10px 20px;background:#17131d;border-bottom:3px solid #9147ff;color:#fff;font-family:Arial,sans-serif}.vz-deadline div{min-width:0}.vz-deadline strong,.vz-deadline span{display:block}.vz-deadline strong{font-size:12px}.vz-deadline span{margin-top:3px;color:#cfc9d7;font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.vz-deadline time{flex:none;color:#c9ee78;font-size:15px;font-weight:800;font-variant-numeric:tabular-nums}iframe{display:block;width:100%;height:${frameHeight};border:0;background:#fff}noscript{position:fixed;inset:0;display:grid;place-items:center;padding:30px;font:16px Arial,sans-serif}@media(max-width:620px){.vz-deadline{height:76px;padding:9px 13px;gap:10px}.vz-deadline span{max-width:210px}.vz-deadline time{font-size:12px}iframe{height:${deadlineAt ? "calc(100% - 76px)" : "100%"}}}</style></head><body>${deadlineBar}<iframe id="vz-plan-frame" data-vizantu-plan-frame data-src="${documentUrl}" title="${title}" sandbox="allow-scripts allow-forms allow-modals allow-downloads allow-popups allow-popups-to-escape-sandbox"></iframe><noscript>Ative o JavaScript para visualizar este plano.</noscript><script nonce="${nonce}" data-vizantu-reviewer-host>${reviewerHostScript(slug, deadlineAt)}</script></body></html>`;
